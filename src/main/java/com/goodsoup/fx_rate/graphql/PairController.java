...
public class PairController {
    ...
    public HistoricalConnection pairHistoricals(PairEntity pair, @Argument Integer first, @Argument String after) {
        ...
        if (decoded == null) {
            itemsPlusOne = historicalRepository.findByPair_IdOrderByDateDescIdDesc(pair.getId(), PageRequest.of(0, size + 1));
        } else {
        ...
        int totalCount = historicalRepository.countByPair_Id(pair.getId());
    ...
    public FileUploadConnection pairFileUploads(PairEntity pair, @Argument Integer first, @Argument String after) {
        ...
        if (decoded == null) {
            itemsPlusOne = fileUploadRepository.findByPair_IdOrderByCreatedAtDescIdDesc(pair.getId(), PageRequest.of(0, size + 1));
        } else {
        ...
        return new FileUploadConnection(totalCount, edges, new PageInfo(endCursor, hasNextPage));
    }
